...
public class Buffer_circ2 implements Tampon {
	...
	public Message get(_Consommateur arg0) {
		...
		synchronized(_lockC){
			...
			if(_att == 0 || _nc > 0)
			{
				...
				try {
					Buffer_circ2._lockC.wait();
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				if(_att > 1 || _att == 1 && _buff[_S].nbExemplairesRestants()>1) Buffer_circ2._lockC.notify();
				else if(_closed) Buffer_circ2._lockC.notifyAll();
		...
		synchronized(_lockP){ if(b && enAttente()==_size-1)Buffer_circ2._lockP.notify(); }
		return ret;
	}
	...
	public void putX(_Producteur arg0, GMessage arg1) {
		...
		synchronized(Buffer_circ2._lockP){
			...
			if(_size - _att == 0 || _np > 0)
			{
				...
				try {
					Buffer_circ2._lockP.wait();
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				if(_size - _att > 1) Buffer_circ2._lockP.notify();
				_np--;
			}
		...
		synchronized(_lockC){ if(enAttente() == 1) Buffer_circ2._lockC.notify(); }
		synchronized(xmas){
			if(xmas.nbExemplairesRestants()!=0)
